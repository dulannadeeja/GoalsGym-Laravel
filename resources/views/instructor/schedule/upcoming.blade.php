        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <ul class="space-y-6 mb-8">
                @forelse($scheduledClasses as $class)
                    <li class="flex justify-between gap-10 sm:gap-20 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6 text-gray-900 dark:text-gray-100">
                        <div class="flex gap-5 sm:gap-10">
                            ...
                            <div class="basis-2/3">
                                ...
                                <div class="flex gap-5 items-center">
                                    <p class="text-lg"><span class="text-2xl font-semibold text-amber-600">Conducted by:</span> {{$class->instructor->name}}</p>
                                    <p class="text-lg"><span class="text-2xl font-semibold text-amber-600">Duration:</span> {{ $class->classType->duration}} miniutes</p>
                                </div>
                            </div>
                        </div>
                        <div>
                            <form action="{{ route('schedule.destroy', $class->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <div class="text-lg max-w-7xl mx-auto sm:px-6 lg:px-8 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6 text-gray-900 dark:text-gray-100">
                        {{ __("You don't have any classes scheduled yet!") }}
                    </div>
                @endforelse
            </ul>
